...
classification, one must select samples of data represeting each classification.
In this analysis, the recorded data is used as a proxy the
`background' labeled data set, and the simulation of the $W$ boson decay is the
`signal'labeled data set. Distributions for each kinematic variable are created
from each labled data set, partitioned as described in
Section~\ref{sec:dataset_part_2}. 

Following the discussion in Section~\ref{sec:dataset_part_2}, care is needed in
forming the probability distribution functions used in classification. For
example, for a particular muon track, perhaps the RPC or FVTX subsystems were
not triggered--the data from these systems may not be available for that
particular event. In that case, a selection process is superimposed such PDFs
are constructed only from events (simulated or otherwise) containing valid data.
This selection process must also be preserved when looking up what PDFs to
ultimately use in calculating the likelihood of an event being generated from a
signal process. This process is represented in
