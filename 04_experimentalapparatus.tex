...
Scientists at RHIC have engineered many ingenious ways to create and maintain
...
RHIC's luminosity and beam polarization have been continuously improving
...
After the ions are sufficiently polarized and filled in the AGS, they are
...
For each event, particles which interact with the detector material are
...
The three tracking stations of the Muon Tracker are utilized for momentum and
...
Since the MuID will fire on Muons with a 2.5 $GeV$ momentum threshold, a high
...
The RPCs are modular. The larger RPC 3 North and South were separated
...
Once the bottom gas gap was installed and secured, the copper readout strips
...
Finally, the high voltage cables were grounded to the chassis and soldered to
...
The PHENIX DAQ can accommodate 32 different triggers. Any transduced signal by a
...
There was one global physics trigger configuration used in the Run 13 data set,
it was called 'PP510Run13'. An example configuration is shown in
Table~\ref{tab:typical_run}. In the case of the $W$ Analysis, the relevant
physics triggers were recorded without pre-scale, indicating that most triggered
$W$ events were actually recorded.
...
\begin{sidewaystable}
    ...
    \begin{tabular}{l c c c c}
      ...
      \textbf{ RPC} \\
      \midrule
      BBCLL1($>$0 tubes) & $<$30cm vertex & - & - & - \\
      BBCLL1($>$0 tubes) narrowvtx & $<$15cm vertex & - & - & - \\
      BBCLL1($>$0 tubes) novertex & any vertex & - & - & - \\
      (MUIDLL1\_N1D$||$S1D)\&BBCLL1(noVtx) & any vertex & N$||$S lastGap $>= 1$ & - & - \\

      ((MUIDLL1\_N2D$||$S2D)$||$(N1D\&S1D))\&BBCLL1(noVtx) & any vertex & 
    \begin{tabular}[c]{@{}c@{}}(N\&S lastGap $>= 1$) $||$\\(N$||$S lastGap $>= 2$)\end{tabular} & - & - \\
...
For example, consider a simplified four-bit version of this scheme with four
