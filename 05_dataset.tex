...
\begin{table}[ht]
  ...
  \caption{
    ...
    as well as the corresponding luminosity currently available.
  }
...
\section{Delete or move the following:}
...
Section~\ref{sec:likelihood}. We use this engineered feature to apply cuts on
the data set to obtain a slightly more even population of signal muons and
background muons. This is important, as we use unbinned maximum likelihood
fitting to differentiate between signal and background events later in the
analysis - if there is a vast population of background events, then they will
mask out any effect in distributions caused by signal events.
